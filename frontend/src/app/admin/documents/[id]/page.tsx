...
import { MarkdownEditor, useAutoSave } from "@/components/admin/markdown-editor";
...
export default function EditDocumentPage({ params }: PageProps) {
  ...
  const treeNodes = useMemo(
    () => convertToTreeNodes(directories),
    [directories, convertToTreeNodes],
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [dirData] = await Promise.all([directoryApi.getTree(), loadDocument(parseInt(id))]);
