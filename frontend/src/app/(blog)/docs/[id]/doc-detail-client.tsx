...
export function DocDetailClient({ docId }: { docId: number }) {
  ...
  const tocItems = useMemo(
    () => (doc?.html ? extractHeadings(doc.html) : []),
    [doc?.html],
  );
  const processedHtml = useMemo(
    () => (doc?.html ? addHeadingIds(doc.html) : ""),
    [doc?.html],
  );
      ...
      <main className="island-main">
        <div className="island-container island-page">
          ...
          <section className="grid gap-4 lg:grid-cols-[290px_1fr] xl:grid-cols-[290px_1fr_250px]">
            <aside
              className={cn(
                "hidden lg:sticky lg:top-[6.6rem] lg:block lg:self-start",
                sidebarOpen &&
            ...
            >
              <div className="island-panel max-h-[calc(100vh-8rem)] overflow-hidden">
                <div className="border-b border-[var(--is-border)] p-4">
                  <h2 className="text-sm font-medium text-[var(--is-text-muted)]">
                    知识目录
                  </h2>
                  <div className="mt-3">
            ...
            <aside className="hidden xl:block">
              {tocItems.length > 0 && (
                <div className="island-panel island-toc p-4">
                  <h3 className="mb-2 text-sm font-medium text-[var(--is-text-muted)]">
                    目录导航
                  </h3>
