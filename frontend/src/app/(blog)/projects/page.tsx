          ...
          {projects.map((project) => {
            ...
            return (
              ...
              >
                ...
                {project.description ? (
                  <p className="line-clamp-5 text-xs leading-6 opacity-90 font-bold">
                    {project.description}
                  </p>
                ) : null}
                <div className="mt-auto flex flex-wrap gap-2 border-t border-black/10 pt-4">
                  {project.github_url ? (
                    <AIButton
                      type="default"
                      size="small"
                      onClick={() => openExternal(project.github_url!)}
                  ...
                  {project.preview_url ? (
                    <AIButton
                      type="primary"
                      size="small"
                      onClick={() => openExternal(project.preview_url!)}
                      className="font-bold flex items-center gap-1"
                    >
                      <ExternalLink className="h-3.5 w-3.5 mr-0.5 inline" />
                      预览
                  ...
                  {project.download_url ? (
                    <AIButton
                      type="default"
                      size="small"
                      onClick={() => openExternal(project.download_url!)}
                      className="font-bold flex items-center gap-1"
                    >
